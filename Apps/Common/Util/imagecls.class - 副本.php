...
class imagecls
{
	...
	{
		if(!imagecls::isUploadFile($source))
			$succeed = false;
	...
	public function thumb($image,$maxWidth=200,$maxHeight=50,$gen = 0,$interlace=true,$filepath = '',$is_preview = true)
    {
		...
		$paths['filename'] = substr($paths['basename'],0,-4);
			...
			if(empty($filepath))
			{
				$thumbname = UPLOADIMG.''.$paths['filename'].'_'.$maxWidth.'x'.$maxHeight.'.'.$paths['extension'];
			}
			else
				$thumbname = $filepath;
	...
	public function water($source,$water,$alpha=80,$position="0")
    {
        ...
        //图片信息
        $sInfo = imagecls::getImageInfo($source);
        $wInfo = imagecls::getImageInfo($water);
